...
async function train(model, data) {
    ...
    // HINT: Take a look at the MNIST example.
    const [trainXs, trainYs] = tf.tidy(() => {const allen = data.nextTrainBatch(TRAIN_DATA_SIZE);
            return[allen.xs.reshape([TRAIN_DATA_SIZE,28,28,1]),allen.labels]});// YOUR CODE HERE
    // Get the testing batches and resize them. Remember to put your code
    // inside a tf.tidy() clause to clean up all the intermediate tensors.
    // HINT: Take a look at the MNIST example.
    const [testXs, testYs] = tf.tidy(() => {const allen = data.nextTestBatch(TEST_DATA_SIZE);
            return[allen.xs.reshape([TEST_DATA_SIZE,28,28,1]),allen.labels]});// YOUR CODE HERE
